...
async function initializeCandleData(): Promise<void> {
  ...
  try {
    ...
    logger.info(`✅ Сформировано ${count6m} свечей 6m из истории`, LogCategory.SYSTEM);
    ...
    logger.info(`✅ Сформировано ${count24m} свечей 24m из истории`, LogCategory.SYSTEM);
    ...
    logger.success(
      ...
      `  ├─ 1m:  ${stats1m.count}/${stats1m.totalCount} (${stats1m.isFull ? 'ПОЛНЫЙ' : 'частичный'})\n` +
      `  ├─ 6m:  ${stats6m.count}/${stats6m.totalCount} (${stats6m.isFull ? 'ПОЛНЫЙ' : 'частичный'})\n` +
      `  └─ 24m: ${stats24m.count}/${stats24m.totalCount} (${stats24m.isFull ? 'ПОЛНЫЙ' : 'частичный'})`,
      LogCategory.SYSTEM
    );
    ...
    logger.error(
      '❌ Ошибка загрузки исторических свечей',
      LogCategory.SYSTEM,
      { error }
    );
    throw error;
...
function showSystemStats(): void {
  ...
  logger.info(
    ...
    `  └─ Давление: Bid ${obStats.bidPercent?.toFixed(1)}% / Ask ${obStats.askPercent?.toFixed(1)}%\n` +
    '\n' +
    '🕯️ Свечи:\n' +
    `  ├─ 1m:  ${stats1m.count}/${stats1m.totalCount} (last: ${stats1m.lastClose?.toFixed(2) || 'N/A'})\n` +
    `  ├─ 6m:  ${stats6m.count}/${stats6m.totalCount} (last: ${stats6m.lastClose?.toFixed(2) || 'N/A'})\n` +
    `  └─ 24m: ${stats24m.count}/${stats24m.totalCount} (last: ${stats24m.lastClose?.toFixed(2) || 'N/A'})\n` +
    '\n' +
    '🔄 Буферы агрегации:\n' +
...
async function main(): Promise<void> {
  try {
    ...
    logger.info(`Торговая пара: ${config.trading.symbol}`, LogCategory.SYSTEM);

    logger.info('💰 Проверка баланса...', LogCategory.SYSTEM);
    const balances = await gateioClient.getBalance();
    logger.success(`✅ Баланс получен: ${balances.length} валют`, LogCategory.SYSTEM);

    logger.info('📊 Получение данных рынка...', LogCategory.SYSTEM);
    const ticker = await gateioClient.getTicker(config.trading.symbol);
    logger.success(
      `✅ Текущая цена ${config.trading.symbol}: ${ticker.last}`,
      LogCategory.SYSTEM
    );
    ...
    logger.success('💪 ПОРВЁМ GATE.IO К ЧЕРТЯМ СОБАЧЬИМ!!!', LogCategory.SYSTEM);
    ...
    logger.error('💥 Критическая ошибка запуска', LogCategory.SYSTEM, { error });
    process.exit(1);
  }
}
