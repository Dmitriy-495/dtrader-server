...
export class GateioClient {
   ...
   * });
   */
  public async getCandles(params: CandleParams): Promise<Candle[]> {
    if (!isValidCurrencyPair(params.currencyPair)) {
      throw new Error(`Некорректная торговая пара: ${params.currencyPair}`);
    }

    logger.debug(
      `Запрос свечей ${params.currencyPair} (${params.interval})...`,
      LogCategory.EXCHANGE
    );

    const candles = await this.request<Candle[]>(
      "GET",
    ...
    logger.debug(`Получено ${candles.length} свечей`, LogCategory.EXCHANGE);
